...
class PlotConditionPlot(plot_base.BasePlot):
    ...
    def plot(self,
        ...
        # fill the grid
        fig, ax = self._fill_grid(data, condition, clim, cm, fig, ax, lookup, sizey, sizex, timepoints,
                                  wells_per_timepoint, nox, noy, plot_map, relative)
    ...
    def _fill_grid(self, data, condition, clim, cm, fig, ax, lookup, sizey, sizex, timepoints, wells_per_timepoint, nox,
                   noy, plot_map, relative):
        """
        Fills the grid with heatmap plots of the spheres
        """
        for tpit in range(0, len(timepoints)):
            count = 0
            images = list(lookup.loc[(lookup[db.conditions.condition_name.key] == condition) & (
                        lookup[db.conditions.timepoint.key] == timepoints[tpit])][db.images.image_id.key])
            # print(str(tpit)+"---------")
            for well in images:
                # print(well)
                ...
                cax = self.heatmask.do_heatplot(img, ax=curax, colorbar=False, crange=clim, update_axrange=False,
                                                cmap=cm)
                curax.set_aspect(1)
                curax.tick_params(
                ...
                scalebar = ScaleBar(0.000001, location=4, color='white', pad=0.1, frameon=False,
                                    font_properties={'size': 5})  # 1 pixel = 0.2 meter
                ...
                if relative:
                    ...
                    cur_h = \
                    self.data.main_session.query(db.masks.shape_h).filter(db.masks.image_id == str(well)).first()[0]
                    cur_w = \
                    self.data.main_session.query(db.masks.shape_w).filter(db.masks.image_id == str(well)).first()[0]
                    xdiff = max_w - cur_w
                    ydiff = max_h - cur_h
    ...
    def _prepare_grid(self, sizey, sizex, timepoints, wells_per_timepoint, nox, noy, plot_map, relative):
        fig, ax = plt.subplots(nrows=sizey, ncols=sizex, subplot_kw={'xticks': [], 'yticks': [], 'aspect': 'equal'},
                               figsize=(50, 6))  # ,squeeze=True, figsize=(28, 6))
    ...
    def _generate_wells(self):
        ...
        conditions = conditions.drop_duplicates(
            [db.conditions.bc_plate.key, db.conditions.bc_x.key, db.conditions.bc_y.key], keep='first')
    ...
    def _get_data(self, metal, filtertuples, filters):
        ...
        if filters is not None:
            for filtername in filters:
                q = q.join(db.object_filters).filter(
                    sa.and_(db.object_filters.FilterName == filtername, db.object_filters.filter_value == 1))
        data = pd.read_sql_query(q.statement, self.data.db_conn)
        return data
