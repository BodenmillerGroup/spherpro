...
class CalculateDistRim(object):
    ...
    def _write_tables(self, filterdata):
        ...
        done = self.data.main_session.query(db.ref_planes.channel_name).filter(
            db.ref_planes.channel_name == CHANNELNAME).count()
        if done > 0:
        ...
        filterdata = filterdata[[db.images.image_id.key, db.objects.object_number.key, db.objects.object_id.key,
                                 db.measurement_names.measurement_name.key, db.ref_planes.ref_plane_number.key,
                                 db.object_measurements.value.key]]
